...
drop table if exists Subscribe_Scholar;
...
alter table Patent_Possess add constraint FK_PatentPossess foreign key (PatentId)
    references Patent (PatentId) on delete restrict on update restrict;

alter table Patent_Possess add constraint FK_ProjectPossess2 foreign key (OwnerID)
    references User (UserID) on delete restrict on update restrict;
...
alter table Subscribe_Scholar add constraint FK_SubscribeScholar foreign key (FanID)
    references User (UserID) on delete restrict on update restrict;

alter table Subscribe_Scholar add constraint FK_SubscribeScholar2 foreign key (ScholarId)
